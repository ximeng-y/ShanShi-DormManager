...
int school::add_student(const student& student_to_add)//添加学生
{
	if (!begin_persistent_mutation()) return persistence_error_code;
	const int result = studentmanager::instance().add(student_to_add) ? 1 : 0;
...
int school::add_dorm(const dorm& dorm_to_add)//添加宿舍并校验楼级约束
{
	if (!begin_persistent_mutation()) return persistence_error_code;
	const auto operation = [&]() -> int {
	if (!dorm_to_add.is_empty())
		return 0;//禁止携带预填住客入库，所有入住必须经过school闭环
	...
	if (dorm_to_add.get_for_gender() != 0 && !b->accepts_gender(dorm_to_add.get_for_gender()))
		return 0;//空宿舍预设性别锁必须被所在楼接纳
	return dormmanager::instance().add_dorm(dorm_to_add) ? 1 : 0;
	};
	const int result = operation();
	return finish_persistent_mutation(result, result == 1);
}
...
int school::add_dorm(int building_id, int floor, int room_num, int max_num, int gender_lock)//扩层与新增宿舍统一事务
{
	...
	const int dorm_add_result = initialized ? add_dorm(new_dorm) : 0;
	if (dorm_add_result < 0)
		return dorm_add_result;
	bool dorm_added = dorm_add_result == 1;
	...
	if (dorm_added)
	{
		const int remove_result = remove_dorm(building_id, dorm_id);
		if (remove_result < 0)
			return remove_result;
		dorm_removed = remove_result == 1;
	}
...
int school::remove_dorm(int building_id, int dorm_id)//删除宿舍并同步清退住客
{
	if (!begin_persistent_mutation()) return persistence_error_code;
	const auto operation = [&]() -> int {
...
int school::remove_building(int building_id)//删除宿舍楼并级联处理楼内宿舍
{
	if (!begin_persistent_mutation()) return persistence_error_code;
	const auto operation = [&]() -> int {
	if (!check::is_valid_building_id(building_id) || buildingmanager::instance().get(building_id) == nullptr)
	...
	for (const auto& key : keys)
		if (key.first == building_id && !is_dorm_consistent(key.first, key.second))
			return 0;//先全量预检，避免级联删除部分提交
	for (const auto& key : keys)
	{
		if (key.first != building_id)
			continue;
		const int remove_result = remove_dorm(key.first, key.second);
		if (remove_result < 0)
			return remove_result;
		if (remove_result == 0)
			return 0;
	}
	return buildingmanager::instance().remove_building(building_id) ? 1 : 0;
	};
	const int result = operation();
	return finish_persistent_mutation(result, result == 1);
}
...
int school::move_student_to_dorm_impl(int building_id, int dorm_id, int student_id, int bed_id, bool specified_bed, bool reset_source_gender)//调宿、换床与原宿舍解锁共享实现
{
	if (!check::is_valid_building_id(building_id) || !check::is_valid_dorm_id(dorm_id) || !check::is_valid_student_id(student_id))
		return -1;
	studentmanager& sm = studentmanager::instance();
	...
	const building* source_building = buildingmanager::instance().get(old_building_id);
	const int old_source_gender = source->get_for_gender();
	if (reset_source_gender && (same_dorm || source_building == nullptr || source_building->get_for_gender() != 3
		|| source->get_current_num() != 1 || old_source_gender == 0))
		return -11;
	if (same_dorm)
	{
	...
	if (reset_source_gender && dm.set_dorm_gender(old_building_id, old_dorm_id, 0) != 1)
	{
		const bool removed_target = dm.remove_student_from_dorm(building_id, dorm_id, student_id) == new_bed_id;
		const bool restored_target_gender = dm.set_dorm_gender(building_id, dorm_id, old_target_gender) == 1;
		const bool restored_source = dm.add_student_to_dorm(old_building_id, old_dorm_id,
			student_id, s->get_gender(), old_bed_id) == old_bed_id;
		const bool restored_source_gender = dm.set_dorm_gender(old_building_id, old_dorm_id, old_source_gender) == 1;
		const bool restored_info = sm.assign_dorm_info(student_id, old_bed_id, old_dorm_id,
			old_building_id, source->get_floor()) == 1;
		return removed_target && restored_target_gender && restored_source && restored_source_gender
			&& restored_info && is_dorm_consistent(old_building_id, old_dorm_id)
			&& is_dorm_consistent(building_id, dorm_id) ? -11 : -10;
	}
	return new_bed_id;
}
...
{
	if (!begin_persistent_mutation()) return persistence_error_code;
	const int result = move_student_to_dorm_impl(building_id, dorm_id, student_id, 0, false, false);
	return finish_persistent_mutation(result, result > 0);
}
...
{
	if (!begin_persistent_mutation()) return persistence_error_code;
	const int result = move_student_to_dorm_impl(building_id, dorm_id, student_id, bed_id, true, false);
	return finish_persistent_mutation(result, result > 0);
}

int school::move_student_to_dorm_reset_source_gender(int building_id, int dorm_id, int student_id, int bed_id)//调宿并解除变空原宿舍的性别锁
{
	if (!begin_persistent_mutation()) return persistence_error_code;
	const bool specified_bed = bed_id != 0;
	const int result = move_student_to_dorm_impl(building_id, dorm_id, student_id, bed_id, specified_bed, true);
	return finish_persistent_mutation(result, result > 0);
}
...
bool school::write_sample_data_plan(const sampledataplan& plan)//按固定计划写入全校数据
{
	...
	for (const sampledormplan& item : plan.dorms)
	{
		...
		if (!new_dorm.set_building_id(item.building_id) || !new_dorm.set_id(item.dorm_id)
			|| !new_dorm.set_max_num(item.max_num) || add_dorm(new_dorm) != 1) return false;
	...
	for (const samplestudentplan& item : plan.students)
	{
		...
		if (!new_student.set_id(item.id) || !new_student.set_name(item.name) || !new_student.set_gender(item.gender)
			|| !new_student.set_grade(item.grade) || !new_student.set_class_num(item.class_num) || add_student(new_student) != 1) return false;
	}
	for (const samplestudentplan& item : plan.students)
...
persistence_start_status school::initialize_persistence()//启动加载或创建持久化文件
{
	...
	const auto archive_invalid_directory_files = [](const QString& directory, QString& error) {
		schoolstorage inspected_storage;
		inspected_storage.set_data_directory(directory);
		schoolsnapshot ignored;
		QString primary_error;
		QString backup_error;
		const storage_load_status primary_status = inspected_storage.load_primary(ignored, &primary_error);
		const storage_load_status backup_status = inspected_storage.load_backup(ignored, &backup_error);
		QStringList archive_errors;
		if (primary_status == storage_load_status::invalid
			&& !inspected_storage.archive_invalid_file(inspected_storage.primary_file_path(), nullptr, &primary_error))
			archive_errors.append(primary_error);
		if (backup_status == storage_load_status::invalid
			&& !inspected_storage.archive_invalid_file(inspected_storage.backup_file_path(), nullptr, &backup_error))
			archive_errors.append(backup_error);
		if (!archive_errors.isEmpty())
			error = archive_errors.join(QLatin1Char('\n'));
		return archive_errors.isEmpty();
	};
	...
	if (executable_exists && fallback_exists)
	{
		...
		QString executable_archive_error;
		QString fallback_archive_error;
		archive_invalid_directory_files(executable_directory, executable_archive_error);
		archive_invalid_directory_files(fallback_directory, fallback_archive_error);
		persistence_error = QStringLiteral("程序目录和用户目录中的数据均无法读取。\n程序目录：%1\n用户目录：%2")
			.arg(executable_error, fallback_error);
		QStringList archive_errors;
		if (!executable_archive_error.isEmpty())
			archive_errors.append(executable_archive_error);
		if (!fallback_archive_error.isEmpty())
			archive_errors.append(fallback_archive_error);
		const QString archive_error = archive_errors.join(QLatin1Char('\n'));
		if (!archive_error.isEmpty())
			persistence_error += QStringLiteral("\n损坏文件保留失败：%1").arg(archive_error);
		read_only = true;
	...
	if (executable_exists)
	{
		...
		const bool source_applied = source_status == storage_load_status::loaded && apply_loaded_snapshot(source_snapshot);
		if (source_applied)
		{
		...
		if (source_status == storage_load_status::loaded)
			persistence_error = QStringLiteral("程序目录数据通过文件校验，但未能完整建立内存状态。系统已进入只读安全模式。");
		else
...
bool school::resolve_persistence_conflict(bool use_executable_data)//确认双目录冲突使用哪份数据
{
	...
	if (!candidate_valid)
		return false;
	const QString selected_directory = use_executable_data
		? schoolstorage::executable_data_directory() : schoolstorage::fallback_data_directory();
	const persistence_start_status normal_status = use_executable_data
		? persistence_start_status::ready : persistence_start_status::fallback_ready;
	return load_storage_directory(selected_directory, writable, !use_executable_data, normal_status);
}
