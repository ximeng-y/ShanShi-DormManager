...
宿舍管理系统（ShanShiDormManager）——C++/Qt6 Widgets 桌面应用，为程序设计实习课程作业。当前领域模型、三个 manager 与顶层 `school` 业务层已建立；Qt Widgets 正式前端已接入日常业务、高级批量调整和样例数据替换闭环。
...
- 样例数据支持追加和“清空现有数据后生成”：两种模式均先形成可复现计划；替换模式由 `school` 保存全校快照并统一清空、写入和失败恢复。
...
`school` 当前已覆盖课程作业所需的基础领域能力，并已作为正式 UI 的统一业务入口；不代表已经完成持久化、权限等完整学校管理系统能力。

## UI 第一、二阶段已实现

正式前端不继承中期演示分支的“API 方法按钮墙”结构，按用户任务组织为以下页面：

- `OverviewPage`：展示学生、入住、空床和楼栋统计，按楼栋汇总宿舍容量，并提供追加式随机样例数据入口。
- `StudentPage`：学生目录、学号/姓名/班级/入住状态筛选、可收起详情面板、双击或回车打开完整只读详情；支持新增、基础资料修改、性别纠错和退学籍。
- `DormResourcePage`：楼栋、宿舍、床位三级横向浏览；支持楼栋和宿舍新增、属性修改与删除。床位使用 `bedtablemodel + QTableView` 轻量展示，避免大量床位控件撑高页面或占用过多内存；选中空床可直接安排入住，选中住客可办理退宿、可视化同宿舍换床或选择其他宿舍调宿。
- `AccommodationPage`：按任务提供系统推荐/随机/指定宿舍/指定床位入住、保留学籍退宿、调宿/同宿舍换床及两名学生床位互换。
...
- `BatchClearDialog`：支持指定宿舍、指定宿舍楼和全部宿舍清退，并选择保留或解除宿舍性别锁。
- `DormAdjustmentDialog`：提供两间宿舍调整前后微型床位图、四类整体调整方式和逐人变化明细。
...
- 前端新增或修改宿舍时将单间床位输入限制为 `1~99`，避免通过 UI 触发异常大的 `QVector` 分配；核心 `dorm` 当前仍只校验床位数大于 0。
- 页面在 `showEvent()` 中刷新只读数据，确保跨页面业务操作后显示最新状态；学生资料处于编辑状态时跳过自动刷新。
- `sampledatagenerator` 是前端辅助服务：按随机种子规划新楼号、4/6 人间、混合楼房间锁、唯一学号、中文姓名与住宿目标；不得直接访问 manager，也不得调用会影响既有未入住学生的全校补分/重排接口。
- 默认样例配置生成 2 栋男生楼、2 栋女生楼、1 栋混合楼；每栋 3 层、每层 4 间，含 8 间四人间和 4 间六人间；男女各 56 人、各 40 人入住，并保留未入住学生和未锁定空宿舍用于验证后续操作。
- 追加生成执行顺序为楼栋、宿舍、房间锁、学生、入住关系；失败补偿按学生、宿舍、楼栋逆序进行。补偿不完整时必须使用严重错误弹窗报告残留数量并提示暂停后续操作。
