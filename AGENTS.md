...
山东石油化工学院宿舍管理系统（ShanShiDormManager）——C++/Qt6 Widgets 桌面应用，为 2025 级程序设计实习课程作业。当前领域模型、三个 manager 与顶层 `school` 基础业务层已建立，学生、宿舍、宿舍楼的常用维护、查询、统计和住宿调整入口基本完备；Qt Widgets 正式前端第一阶段已接入日常业务闭环，高级批量调整保留为第二阶段能力。
...
- 样例数据生成当前只允许追加：必须先形成可复现计划并经 `school` 公开入口写入，失败时只补偿本次新增对象；“清空后生成”仅保留禁用入口，须等前端 Phase 2 收尾后再实现。
...
- `src/ui/`：Qt Widgets 正式前端，包括应用主框架、统一样式与反馈组件、概览页、学生管理页、宿舍资源页、住宿任务页、高级调整说明页、样例数据生成器及对应业务弹窗。
...
- `AdvancedAdjustmentPage`：只说明第二阶段批量分配、全校重排、批量清退和宿舍整体交换能力，不提供未接入的假按钮。
...
- UI 第二阶段接入批量分配、全校重排、批量清退与宿舍整体交换。
- 前端 Phase 2 收尾后实现样例数据“清空现有数据后生成”；实现前必须设计原数据快照与失败整体恢复，不得直接启用当前禁用入口。
