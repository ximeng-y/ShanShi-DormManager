...
山东石油化工学院宿舍管理系统（ShanShiDormManager）——C++/Qt6 Widgets 桌面应用，为 2025 级程序设计实习课程作业。当前领域模型、三个 manager 与顶层 `school` 协调骨架已建立，UI 层仍仅占位，尚无业务逻辑接入。
...
- `school`：入住/退宿、退学籍、性别纠错、建删宿舍、删楼、修改楼属性、可用宿舍查询、批量分配、清空宿舍、跨宿舍交换等跨聚合事务。
...
- 全校随机补分、清空后重新分配。
- 清空全部宿舍（保留/放开房间锁）。
- 两间同锁宿舍整体交换、重叠交换、多余住客离宿、混宿楼男女宿舍交换；修改前保存床位快照，失败时恢复原床位和房间锁。

`school` 当前目标是使架构和已有领域能力自洽，不代表已经完成所有学校业务。
...
- `dorm::swap_student` 和 `shuffle_beds` 只修改本间床位；若未来作为公开业务使用，必须由 `school` 包装并同步 student 的 `bed_id`。
